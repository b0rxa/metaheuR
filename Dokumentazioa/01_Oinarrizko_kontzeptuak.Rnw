...
Metodo heuristikoak oso interesgarriak ziren, baina desegokiak problema berrietan berrerabiltzeko. Hori dela eta, 1975etik aurrera beste hurbilketa batzuk, bilaketa heurisktikoak edo metaheuristikak deritzenak, garatzen hasi ziren. Hona hemen adibide eta data batzuk:
...
Hortaz, helburu funtzioa --ibilbidearen distantzia totala, alegia-- 225.8 da. Ikus dezagun adibidea R-n. Lehenik eta behin, \code{metaheuR} paketea kargatu eta problemaren helburu funtzioa sortuko dugu:
...
Ausazko soluzio bat sortzen dugu, bakarrik 3 kolore erabiliz. Kontutan hartu behar da soluzioa \code{factor} motako bektore bat izan behar dela, non balio posibleen kopurua nodo kopuruaren berdina izan behar den (kasurik txarrenean, grafo osoa denean, nodo bakoitzak kolore ezberdin bat izan beharko du). Ausazko soluzioa ea bideragarria denetz egiaztatuko dugu, problema definitzean \code{is.valid} sortutako funtzioa erabiliz.
...
Algoritmo hau \code{metaheuR} paketean inplementaturik dago, diferentzia txiki batekin; lehendabiziko hiria, erabiltzaileak sartu beharrean, algoritmoak aukeratzen du matrizean dagoen baliorik txikienari erreparatuz. Ikus dezagun algoritmoaren inplementazioa. Funtzioak parametro bakar bat jasotzen du, \code{cmatrix}, kostu matrizea dena; hasteko, aukeraezinak diren balioak adierazteko \code{NA}-k txertatuko ditugu matrizean. Kontutan hartuz diagonalean dauden balio guztiak ezin direla aukeratu (ez du zentzurik hiri batetik hiri berberara joatea), aukeraezin gisa finkatzen ditugu eta, gero, matrizean dagoen elementurik txikiena(k) aukeratuko d(it)ugu.
...
Une honetan \code{solution} bektoreak eraikitako soluzio bat gordetzen du. Dena dela, soluzioa permutazio baten bidez kodetu nahi dugunez, funtzioaren amaieran ondoko kodea izango dugu.
...
Funtzioa grure problemari aplikatzen badiogu, hona hemen emaitza:
...
Lortzen dugun soluzioa ezberdina da, baina bere kostua, ordea, berdina. Izan ere, nahiz eta soluzioa ezberdina izan, definitzen duen zikloa berdina da, beste noranzkoan izanda ere. Beste era batean esanda, goiko kodean dugun soluzioa atzetik aurrera irakurtzen badugu, lehen bilatutako soluzio berbera dugu!.
...
Ikus dezagun adibide bat. Demagun $P=\{2,6,3,6,3\}$, $W=\{1,3,1,10,2\}$ eta $c_m = 5$ balio bektorea, pisu bektorea eta motxilaren edukiera maximoa direla hurrenez hurren. Motxilan elementu guztiak sartzen dituen soluzioa bideraezina da, pisu totala (8) limitea baino handiagoa baita.
