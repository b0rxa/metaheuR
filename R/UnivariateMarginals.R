...
setValidity(
  ...
  method = function(object){
    m = object@prob.table
    if (!all(object@prob.table<=1 & object@prob.table>=0)) stop("All the values in the prob table have to be between 0 and 1")
    
    if (object@binary){
      if (nrow(object@prob.table)!=1) stop("For models over logical variables the 'prob.table' should only contain one row, representing the probability of the variable being 'TRUE'")
    }else{
      ...
      if (nrow(object@prob.table)<2) stop("For factor models there should be at least two possible values")
      ## Note that, due to rounding problems, the summ may not be exactly 1
      if (max(colSums(object@prob.table)-1) > 1/(100*nrow(object@prob.table))) stop("The column-wise sums in 'prob.table' should be 1")
    }
    return (TRUE)
  }
)
...
setMethod(
  ...
  definition = function(object , nsim = 1 , seed = NULL , ...) {
    ...
    if (object@binary) { ## There is only one value per column, the probability of being TRUE
      get.sample <- function(i) runif(n)<object@prob.table[1,]
    }else{
      get.sample <- function(i){
        levels <- rownames(object@prob.table)
        ## Get a level according to the probabilities
        sample <- sapply(1:n , FUN = function(j) sample(x = levels , 
                                                        size = 1 , 
...
univariateMarginals<-function(data , ...){
  if (class(data)!="list") stop ("The data has to be a list")
  ...
  if (cl == 'factor'){
    ...
    probs <- apply(df , MARGIN=1 , FUN = function(x) table(factor(x, levels))) / length(data)
  }else if (cl == 'logical'){
  ...
  new("UnivariateMarginals",prob.table = probs , binary = cl == "logical")
}
