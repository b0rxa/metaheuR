...
#' A.sol <- rep("A", 10)
#' B.sol <- rep("B", 10)
#' kPointCrossover(sol.1=A.sol, sol.2=B.sol, k=1)
#' kPointCrossover(sol.1=A.sol, sol.2=B.sol, k=2)
#' kPointCrossover(sol.1=A.sol, sol.2=B.sol, k=9) 
#' 
kPointCrossover <- function (sol.1, sol.2, k=1, ...) {
  # Parameter control 
  ...
  if (n < k + 1) {
    warning("The length of the vectors is ", n, " so at most there can be ", n - 1, 
            " cut points. The parameter will be updated to this limit")
    k <- n - 1
  }
  ...
  if(k == n - 1) {
    ...
    # Calculate the size of each chunk. The first chunk will be positions 
    # between 1 and the first cut point,     and so on.
  ...
  # To create new solutions, copy each of the two and replace with the other 
  # those positions in id
  new.sol.1 <- sol.1
...
#' @examples
#' 
#' A.sol <- identityPermutation (10)
#' B.sol <- randomPermutation (10)
#' orderCrossover (sol.1=A.sol, sol.2=B.sol)
#' 
orderCrossover <- function (sol.1, sol.2, ...) {
  # Parameter control
  if(class(sol.1) != 'Permutation' | class(sol.2) != 'Permutation') {
    stop("This crossover operator is for permutation")
  }
  if (length(sol.1) != length(sol.2)) {
    stop("The two permutations should be of the same size") 
  }
  
  n <- length(sol.1)
  if(n < 3) {
    stop("The minimum length to use this crossover operator is 3")
  }
