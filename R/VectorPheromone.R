...
setValidity(
  ...
  method=function(object) {
    ...
    if (object@binary) {
      if (nrow(object@trail) != 2) {
        stop("For models over logical variables the 'trail' parameter should have ",
             "two rows, the first corresponding to the trail associated to the ",
             "'FALSE' value and the other corresponding to the 'TRUE' value" )
      }
    } else {
...
setMethod(
  ...
  definition=function (object, solution, value, ...) {
    ...
    if (object@binary) {
      ...
      # Identify the row corresponding to each value in the solution
      row.id <- sapply(solution, 
                       FUN=function(x) {
                         return(which(levels %in% x))
                       })
    }
...
setMethod(
  ...
  definition=function(object, n=1, seed=NULL, ...) {
    ...
    aux <- object@trail
    # In case the values are negative, transform them so as the smallest one has 
    # the smallest probability and the (inverted) ratios are kept
    if (min(aux) <0) aux <- -1 / aux
    # Normalize
    probs <- scale(aux, center=FALSE, scale=colSums(aux))
    c <- ncol(probs)
    
    # The second column is the one corresponding with TRUE
    if (object@binary) {
      getSample <- function(i) {
        return(runif(c)<probs[2,])
      }
    }else{
      getSample <- function(i) {
...
vectorPheromone <- function(binary=FALSE, initial.trail, evaporation.factor, ...) {
  if (binary & nrow(initial.trail) != 2) {
    stop ("For binary pheromone models the initial matrix has to have two rows")
  }
  if (nrow(initial.trail) < 2) {
    stop ("The initial matrix has to have, at least, two rows")
  }
  if (nrow(initial.trail) > 2 & is.null(rownames(initial.trail))) {
    stop("For matrices with more than two rows the rows have to be named")
  }
  ...
  obj <- new("VectorPheromone", trail=initial.trail, binary=binary, 
