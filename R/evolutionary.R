...
basic.genetic.algorithm<-function (evaluate, initial.population, select.subpopulation ,  selection.ratio , select.cross , cross , mutate , mutation.rate , do.log = TRUE , verbose = TRUE , non.valid='ignore', valid=function(solution){TRUE} , correct=function(solution){solution}, resources = cresource() , ...){
  
  if (selection.ratio <=0 | selection.ratio >= 1) stop("The selection ratio has to be >0 and <1")
  
  if (mutation.rate < 0) warning("The probability of mutation has been set to a negative value. No solution will be mutated")
  
  if (mutation.rate > 1) warning("The probability of mutation has been set to a value greater than 1. All the solutions will be mutated")
  ...
  add.consumed(resources, t = as.numeric(Sys.time() - t0) , ev = evaluations)
  ...
  while(!is.finished(resources)){
    ...
    if (verbose) cat(paste("Running generation " , generation , 
                           ". Current population = " , signif(mean(current.evaluation) , 3), 
                           " +- " , signif(sd(current.evaluation) , 3) , 
                           ". Best solution = " , best.evaluation, "\n" , sep=""))
    
    ## Get the subpopulation
    t0 <- Sys.time()
    selected <- select.subpopulation (current.population ,
                                      current.evaluation ,
                                      subpopulation.size , ...)
    ...
    while(length(new.solutions) < needed.solutions){
      to.cross <- select.cross(population = selected$population , 
                               evaluation = selected$evaluation, 
                               size = 2, ...)$population
      cross.results <- cross(to.cross[[1]] , to.cross[[2]] , ...)
      cross.results <- lapply (cross.results , 
                               FUN = function(x) {if (runif(1)<mutation.rate){
                                 mutate(x , ...)
                               }else{
                                 x
                               }})
      ...
      new.solutions <- lapply(new.solutions , FUN = function(x) check.solutions(x,non.valid,valid,correct))
    }
    new.solutions <- new.solutions [1:needed.solutions]
    ## Evaluate the new solutions
    new.sol.evaluations <- sapply(new.solutions , evaluate)
    
    add.consumed(resources, t = as.numeric(Sys.time() - t0) , ev = needed.solutions , it = 1)
  ...
  ## Build the output
  mhresult(algorithm = "Basic Genetic Algorithm" ,
           description = paste("Basic Genetic Algorithm guided by " , deparse(substitute(evaluate))) ,
           parameters = list(crossover.operator = deparse(substitute(cross)),
                             selector.subpopulation = deparse(substitute(select.subpopulation)),
                             selection.rate = selection.ratio,
                             selector.cross = deparse(substitute(select.cross)),
                             muatation = deparse(substitute(select.cross)),
...
basic.eda <- function (evaluate, initial.population, select.subpopulation ,  selection.ratio , learn , do.log = TRUE , verbose = TRUE , non.valid='ignore', valid=function(solution){TRUE} , correct=function(solution){solution}, resources = cresource() , ...){
  ...
  ## Build the output
  mhresult(algorithm = "Basic EDA" ,
           description = paste("Basic Estimation of Distribution Algorithm guided by " , deparse(substitute(evaluate))) ,
