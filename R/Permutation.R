...
setMethod(
  ...
  definition=function(e1, e2) {
    return(all(e1@permutation == e2@permutation))
  })
...
setMethod(
  ...
  definition=function(x, i, j, drop) {
    return(x@permutation[i])
  })

setMethod(f="length" , 
          signature="Permutation", 
          definition=function(x) {
            return(length(x@permutation))
          })
...
if (!isGeneric("swap")) {
  setGeneric(name="swap", 
             def=function(permutation, i1, i2) {
...
if (!isGeneric("shuffle")) {
  setGeneric(name="shuffle", 
             def=function(permutation, ratio=1) {
...
setMethod(
  ...
  definition=function(permutation, ratio=1) {
    ...
    for (i in 1:(n*ratio)) {
      permutation <- swap(permutation, sample(1:n, 1), sample(1:n, 1))
    }
    return(permutation)
  })
...
setMethod(
  ...
  definition=function(permutation, i1, i2) {    
    ...
    return(permutation(permutation@permutation[newpermu]))
  })
...
permutation <- function(vector) {
  obj <- new(Class="Permutation", permutation=vector)
  return(obj)
}

identityPermutation <- function(length) {
  obj <- new(Class="Permutation", permutation=1:length)
  return(obj)
}

randomPermutation <- function(length) {
  obj <- new(Class="Permutation", permutation=sample(1:length))
  return(obj)
}
