...
setValidity(
  ...
  method = function(object){
    ...
    ford.inserts <- do.call(rbind,sapply(1:(n-1), FUN=function(x) cbind((x),(x+1):n)))
    ## Create all possible backward inserts, without taking repetitions into account
    back.inserts <- do.call(rbind,sapply(n:3, FUN=function(x) cbind((x),(x-2):1)))
    ## Matrix with all possible inserts and no repetitions
    inserts <- rbind(ford.inserts,back.inserts)
    if (!all(object@position.list %in% inserts)) stop("Some of the defined inserts are not adequate")
    if (!all(inserts %in% object@position.list)) stop("Not all possible inserts are considered")
...
setMethod(
  ...
  definition = function(neighborhood) {
    if (has.more.neighbors(neighborhood)){
      ...
      nxt <- insert(neighborhood@base,neighborhood@position.list[neighborhood@id,1], neighborhood@position.list[neighborhood@id,2])
...
setMethod(
  ...
  signature = "InsertNeighborhood", 
  definition = function(neighborhood) {
    neighborhood@id <= nrow(neighborhood@position.list)
  })
...
insertNeighborhood<-function(base,random = FALSE){
  ...
  new("InsertNeighborhood",base=base, position.list=positions, random=random, id=1)
}
