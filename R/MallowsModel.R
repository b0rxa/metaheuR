...
setValidity(
  ...
  method = function(object){
    if (!object@distance %in% c('Kendall' , 'Cayley' , 'Hamming' , 'Ulam')) stop ("Non valid distance. Valid options are 'Kendall' , 'Cayley', 'Hamming' and 'Ulam'")
    return (TRUE)
  }
)
...
setMethod(
  ...
  definition = function(object , nsim = 1 , seed = NULL , ...) {
    ...
    f <- function(i) {
      permu <- rmm(n = 1 , sigma0 = s0 , theta = object@theta , dist.name = object@distance)[1,]
      permutation(permu)
    }
...
mallowsModel<-function(data , distance = 'Kendall'  , ...){
  ...
  if (class(data[[1]])!= "Permutation") stop ("The elements in the list have to be objects of class 'Permutation'")
  
  if (!distance %in% c('Kendall' , 'Cayley' , 'Hamming' , 'Ulam')) stop ("Non valid distance. Valid options are 'Kendall' , 'Cayley', 'Hamming' and 'Ulam'")
  
  ## Convert the list into a matrix
  mat <- do.call(rbind , lapply(data , as.numeric))
  model <- lmm(mat , dist.name = distance , ...)
  
  new("MallowsModel",mode = permutation(model$mode) , theta = model$theta , distance = distance)
}
