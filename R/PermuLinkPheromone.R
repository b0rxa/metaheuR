...
setMethod(
  ...
  definition = function (object , solution , value , ...){
    if (class(solution) != "Permutation") stop("This pheromone model is designed for objects of class Permutation")
...
setMethod(
  ...
  definition = function(object , n = 1, seed = NULL , ...) {
    ...
    get.sample <- function (i){
      ...
      if (min(mat)<0) mat <- -1/mat
      n <- ncol(mat)
      # Select one node at random
      from <- sample(1:n , 1)
      sol <- c(sol , from)
      
      # Select the next node, according to the probabilities extracted from the pheromone matrix
      # The 0 in the middle is icluded so as the indices in "prob" are the same as in the permutation; the 0 corresponds to the probability of going from "from" to "from"
      prob <- vector()
      if (from > 1) prob <- c(prob , mat[1:(from - 1) , from])
      prob <- c(prob , 0)
      if (from < n) prob <- c(prob , mat[from , (from+1):n])
      prob <- prob / sum(prob)
      ...
      while (length(sol) < n-1){
        ...
        # Select the new node
        to <- sample (1:n , 1 , prob = prob)
        # Update the matrix to avoid returning to the selected position (rows represent the "from" node and columns the "to" node)
        mat[ , from] <- 0
        mat[from , ] <- 0
      }
...
permuLinkPheromone<-function(initial.trail , evaporation.factor , ...){  
  new("PermuLinkPheromone" , trail = initial.trail , evaporation.factor = evaporation.factor)
}
