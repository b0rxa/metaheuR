...
setMethod(
  ...
  definition = function(object , n = 1, seed = NULL , ...) {
    ...
    get.sample <- function (i){
      ...
      ## Sample the marginal distribution and update the auxiliar matrix to avoid selecting already included values
      for (pos in 1:n){
        prob <- mat[,pos]
        prob <- prob / sum(prob)
...
permuPosPheromone<-function(initial.trail , evaporation.factor , ...){  
  new("PermuPosPheromone",trail = initial.trail , evaporation.factor = evaporation.factor)
}
